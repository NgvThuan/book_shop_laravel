...
//		return \Excel::download(new TransactionInvoiceExport($transaction), 'don-hang.xlsx');
	...
	public function productHistory()
    {
        ...
        $products = Order::with('product')
            ->addSelect('od_product_id')
            ->whereIn('od_transaction_id', function ($query) use ($userId) {
                $query->select('id')->from('transactions')->where('tst_user_id', $userId);
            })
        ...
        return view('user.product_history', $viewData);
    }
}
