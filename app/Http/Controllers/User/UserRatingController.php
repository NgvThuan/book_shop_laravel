...
class UserRatingController extends Controller
{
    ...
    public function addRatingProduct(Request $request)
    {
        if ($request->ajax())
        {
            ...
            if ($rating->id) {
                $html = view('frontend.pages.product_detail.include._inc_rating_item',compact('rating', 'product'))->render();
    ...
    public function staticRatingProduct($productID, $number)
    {
        ...
        if ($product->pro_review_total)
        {
            $product->pro_age_review = floor($product->pro_review_star / $product->pro_review_total);
            $product->save();
        }
    }
}
